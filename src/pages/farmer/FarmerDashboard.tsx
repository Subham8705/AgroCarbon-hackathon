...
export default function FarmerDashboard() {
  ...
  const handlePolygonComplete = (points: [number, number][]) => {
    ...
    if (isOverlapping) {
      alert("Overlap detected! This area intersects with an existing farm boundary.");
      setCurrentPoints([]);
      return;
    }
  ...
  const handleSave = async () => {
    if (polygons.length === 0) {
      alert("Please draw at least one valid closed boundary on the map.");
      return;
    }

    if (formData.trees === '' || formData.yearsFollowed === '' || formData.acres === '') {
      alert("Please fill in all farm practice fields.");
      return;
    }

    if (Number(formData.acres) <= 0) {
      alert("Number of acres must be greater than 0.");
      return;
    }
    ...
    <DashboardLayout role="farmer">
      <div className="p-6 lg:p-8 space-y-8">
        ...
        <div className="animate-fade-in flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-display font-bold text-foreground">Welcome, {user?.name || 'Farmer'}! 🌾</h1>
            <p className="text-muted-foreground mt-1">Track your carbon credits and manage your farm profile</p>
          </div>
          <div>
            {!isEditing ? (
          ...
          <div className="stat-card animate-fade-in" style={{ animationDelay: '0.1s' }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="stat-label">tCO₂ Estimated</div>
              </div>
            </div>
          </div>
          <div className="stat-card animate-fade-in" style={{ animationDelay: '0.2s' }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="stat-label">Land Registered</div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 animate-fade-in">
          ...
          <div className="card-elevated overflow-hidden h-fit">
            {isEditing && (
              <div className="p-4 border-b border-border flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="Search location..." className="input-field pl-9 py-2 text-sm" />
                </div>
                <button onClick={handleSearch} className="btn-secondary py-2 px-3 text-sm">Search</button>
            ...
            <div className="h-[400px] relative">
              ...
              {isEditing && currentPoints.length > 0 && (
                <div className="absolute bottom-4 left-4 right-4 bg-card/90 backdrop-blur rounded p-2 text-sm border shadow z-[1000] flex justify-between">
                  ...
                  <button onClick={handleClearCurrentDrawing} className="text-destructive text-xs font-bold">Clear</button>
                </div>
              )}
              {polygons.length > 0 && (
                <div className="absolute top-4 right-4 bg-card/90 backdrop-blur rounded p-2 border shadow z-[1000]">
                  <div className="text-green-600 flex items-center gap-1 text-sm font-bold"><Check className="w-4 h-4" /> {polygons.length} Areas Set</div>
                  {isEditing && <button onClick={handleResetMap} className="text-xs text-muted-foreground mt-1 flex items-center gap-1"><RefreshCw className="w-3 h-3" /> Clear All</button>}
                </div>
              )}
            </div>
            ...
            <div className="p-4 bg-secondary/30 border-t border-border">
              <div className="flex items-center gap-2 mb-2 text-sm font-medium">
                ...
                Environment Data (Auto-fetched)
              </div>
              <div className="grid grid-cols-3 gap-2 text-center text-sm">
                <div><div className="font-bold text-lg">{envData.soc}</div><div className="text-xs text-muted-foreground">SOC</div></div>
                <div><div className="font-bold text-lg">{envData.ndvi}</div><div className="text-xs text-muted-foreground">NDVI</div></div>
                <div><div className="font-bold text-lg">{envData.rainfall}</div><div className="text-xs text-muted-foreground">Rainfall</div></div>
              </div>
            </div>
          </div>
          ...
          <div className="card-elevated p-6 h-fit space-y-5">
            <h3 className="text-lg font-display font-bold text-foreground">Farm Practices</h3>

            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium">Tillage Practice</label>
                <div className="relative">
                  <select disabled={!isEditing} value={formData.tillage} onChange={e => setFormData({ ...formData, tillage: e.target.value as any })} className="input-field appearance-none disabled:bg-muted disabled:text-muted-foreground">
                    <option value="ploughing">Ploughing (Conventional)</option>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Cover Crop Used?</label>
                <div className="flex gap-3">
                  <button onClick={() => isEditing && setFormData({ ...formData, coverCrop: true })} className={`flex-1 py-2 rounded-lg border text-sm font-medium ${formData.coverCrop ? 'bg-primary/10 border-primary text-primary' : 'bg-background border-border'} ${!isEditing && 'opacity-70 cursor-not-allowed'}`}>Yes</button>
                  <button onClick={() => isEditing && setFormData({ ...formData, coverCrop: false })} className={`flex-1 py-2 rounded-lg border text-sm font-medium ${!formData.coverCrop ? 'bg-primary/10 border-primary text-primary' : 'bg-background border-border'} ${!isEditing && 'opacity-70 cursor-not-allowed'}`}>No</button>
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium flex gap-2 items-center"><TreePine className="w-4 h-4" /> Trees on Farm</label>
                <input
                  type="number"
                  disabled={!isEditing}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Years Practices Followed</label>
                <input
                  type="number"
                  disabled={!isEditing}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Number of Acres</label>
                <input
                  type="number"
                  disabled={!isEditing}
